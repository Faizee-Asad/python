...
class Database:
    ...
    def get_all_tables_for_management(self):
        """Get all tables for management purposes with status."""
        self.cursor.execute('''
            SELECT t.id, t.name, t.capacity,
                   CASE WHEN o.id IS NOT NULL THEN 'occupied' ELSE 'available' END as status
            FROM tables t
    ...
    def add_order_item(self, order_id, product_id, quantity, price):
        """Alias for add_item_to_order for compatibility."""
        return self.add_item_to_order(order_id, product_id, quantity, price)
    ...
    def get_order_history(self, date_filter="All Time"):
        ...
        if date_filter == "Today":
            query += " AND DATE(o.closed_at) = DATE('now')"
        elif date_filter == "Last 7 Days":
            query += " AND o.closed_at >= DATE('now', '-7 days')"
        elif date_filter == "Last 30 Days":
            query += " AND o.closed_at >= DATE('now', '-30 days')"
    ...
    def get_live_stats(self):
        ...
        # Today's revenue
        stats['today_revenue'] = self.get_today_sales()
        ...
        stats['occupied_tables'] = sum(1 for t in tables if t['status'] == 'occupied')
        ...
        ''')
        result = self.cursor.fetchone()
        stats['avg_order_value'] = result['avg_value'] if result['avg_value'] else 0
        ...
        ''')
        result = self.cursor.fetchone()
        stats['top_product'] = result['name'] if result else None
        
        # Active staff (simplified - count all users as we don't track login/logout)
        ...
        for row in self.cursor.fetchall():
            hourly_data[f"{int(row['hour']):02d}:00"] = row['amount']
        stats['hourly_sales'] = hourly_data
        ...
        category_data = {}
        for row in self.cursor.fetchall():
            category_data[row['category']] = row['amount']
        stats['category_distribution'] = category_data
        
        return stats

    # Export-related methods
    def get_daily_sales_details(self):
        """Get detailed daily sales for export."""
        return list(self.get_all_sales_data_for_export())

    def get_monthly_sales_details(self):
        """Get monthly sales summary for export."""
        self.cursor.execute('''
            SELECT 
    ...
    def get_product_sales_details(self):
        """Get product sales details for export."""
        return [dict(row) for row in self.get_top_selling_products(limit=1000)]

    def get_staff_performance_details(self):
        """Get staff performance details for export."""
        return [dict(row) for row in self.get_server_performance()]

    def get_complete_order_history(self):
        """Get complete order history for export."""
        return [dict(row) for row in self.get_all_sales_data_for_export()]
