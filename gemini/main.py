...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.frames = {}
        for F in (LoginScreen, TableScreen, OrderScreen, SettingsScreen, ReportsScreen):
...
class OrderScreen(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.last_settled_order_details = {}
    ...
    def load_order_items(self, is_editable):
        ...
        self.last_settled_order_details = {"items": items, "subtotal": subtotal, "tax": tax, "total": total, 
    ...
    def settle_order(self):
        if not self.last_settled_order_details.get("items"):
    ...
    def _set_post_payment_state(self):
        """Disables editing and shows the print button."""
    ...
    def _set_pre_payment_state(self):
        """Enables editing and shows the settle button."""
        self.print_button.grid_forget()
        self.settle_button.grid(row=4, column=0, columnspan=2, sticky="ew", ipady=10, pady=(10,0))

    def print_last_receipt(self):
        self._print_receipt_logic(self.last_settled_order_details)
...
class ReportsScreen(ctk.CTkFrame):
    ...
    def refresh(self):
        if pd is None or plt is None:
            ...
            ctk.CTkLabel(self.main_frame, text="Required libraries for reporting are not installed.\nPlease run: pip install pandas matplotlib",
                         font=Style.HEADER_FONT, text_color=Style.DANGER).pack(expand=True)
            return
    ...
    def plot_bestsellers(self, df, row, col):
        ...
        ax.set_xlabel('Quantity Sold', color=Style.TEXT_MUTED)
        ax.set_ylabel('')
    ...
    def plot_monthly_sales(self, df, row, col):
        ...
        ax.set_ylabel('Total Sales ($)', color=Style.TEXT_MUTED)
        ax.set_xlabel('')
    ...
    def _create_figure(self):
        fig, ax = plt.subplots()
        fig.set_facecolor(Style.FRAME_BG)
        ax.set_facecolor(Style.BACKGROUND)
        ax.tick_params(axis='x', colors=Style.TEXT_MUTED)
        ax.tick_params(axis='y', colors=Style.TEXT_MUTED)
