...
class Database:
    def __init__(self, db_name="restaurant_pos.db"):
        ...
        # self.conn = sqlite3.connect(db_name) # for creating in exiting dir 
    ...
    def create_tables(self):
        ...
        # Tables table (restaurant floor plan)
        self.cursor.execute('''
    ...
    def seed_initial_data(self):
        ...
        if self.cursor.fetchone()[0] == 0:
            products = [
                ...
                ('Bruschetta', 9.25, 'Appetizers'), ('Calamari', 12.75, 'Appetizers'),
                ('Margherita Pizza', 15.99, 'Mains'), ('Spaghetti Carbonara', 18.50, 'Mains'),
                ('Grilled Salmon', 24.00, 'Mains'), ('Ribeye Steak', 32.50, 'Mains'),
                ('Veggie Burger', 14.75, 'Mains'), ('Tiramisu', 9.50, 'Desserts'),
                ('Chocolate Lava Cake', 10.00, 'Desserts'), ('Cheesecake', 8.75, 'Desserts'),
                ('Coca-Cola', 3.50, 'Drinks'), ('Orange Juice', 4.00, 'Drinks'),
                ('Latte', 5.25, 'Drinks'), ('Sparkling Water', 3.00, 'Drinks')
            ]
    ...
    def get_best_selling_items(self, limit=10):
        self.cursor.execute('''
    ...
    def get_sales_by_server(self):
        self.cursor.execute('''
            SELECT u.username, SUM(oi.quantity * oi.price_at_time) as total_sales
            FROM order_items oi
            JOIN orders o ON oi.order_id = o.id
